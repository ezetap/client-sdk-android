...
public class EzeAPIActivity extends Activity {
            ...
            switch (WORKFLOW_SELECTED) {
                ...
                case EzeAPIConstants.WORKFLOW_DO_WALLETTRANSACTION:
                    if (EzetapUserConfig.getEzeUserConfig() != null)
                        ...
                        ezetapErrorCallBack(EzetapErrors.ERROR_INIT_REQUIRED.getErrorCode(), EzetapErrors.ERROR_INIT_REQUIRED.getErrorMessage());
                    break;
                case EzeAPIConstants.WORKFLOW_DO_CNPTRANSACTION:
                    if (EzetapUserConfig.getEzeUserConfig() != null)
                        initiateCNPPayment(new JSONArray(APIParams));
                    else
                        ezetapErrorCallBack(EzetapErrors.ERROR_INIT_REQUIRED.getErrorCode(), EzetapErrors.ERROR_INIT_REQUIRED.getErrorMessage());
                    break;
    ...
    private int returnWorkflow(String apiName) {
        ...
        if (apiName.equalsIgnoreCase("cnpTransaction"))
            return EzeAPIConstants.WORKFLOW_DO_CNPTRANSACTION;
    ...
    private void initializeUser(JSONArray args) {
        ...
        if (EzetapUserConfig.getEzeUserConfig() == null) {
            ...
            if (jsonConfigData.has(EzeAPIConstants.DEMO_APP_KEY)) {
                try {
                    appKey = jsonConfigData.getString(EzeAPIConstants.DEMO_APP_KEY);
            ...
            if (jsonConfigData.has(EzeAPIConstants.PROD_APP_KEY)) {
                try {
                    prodAppKey = jsonConfigData.getString(EzeAPIConstants.PROD_APP_KEY);
            ...
            if (jsonConfigData.has(EzeAPIConstants.KEY_MERCHANT_NAME)) {
                try {
                    merchantName = jsonConfigData.getString(EzeAPIConstants.KEY_MERCHANT_NAME);
            ...
            if (jsonConfigData.has(EzeAPIConstants.KEY_USER_NAME)) {
                try {
                    userName = jsonConfigData.getString(EzeAPIConstants.KEY_USER_NAME);
            ...
            if (jsonConfigData.has(EzeAPIConstants.KEY_CURRENCY_CODE)) {
                try {
                    currencyCode = jsonConfigData.getString(EzeAPIConstants.KEY_CURRENCY_CODE);
            ...
            if (jsonConfigData.has(EzeAPIConstants.KEY_APP_MODE)) {
                try {
                    appMode = getAppMode(jsonConfigData.getString(EzeAPIConstants.KEY_APP_MODE));
                    if (appMode == null) {
                        ezetapErrorCallBack(EzetapErrors.ERROR_MISSING_MANDATORYPARAMS.getErrorCode(), EzetapErrors.ERROR_MISSING_MANDATORYPARAMS.getErrorMessage());
                        return;
                    }
                    if (appMode == AppMode.EZETAP_PROD) {
                        if (jsonConfigData.has(EzeAPIConstants.PROD_APP_KEY)) {
                            appKey = jsonConfigData.getString(EzeAPIConstants.PROD_APP_KEY);
                            if ("".equalsIgnoreCase(appKey) || appKey == null) {
                                ezetapErrorCallBack(EzetapErrors.ERROR_MISSING_MANDATORYPARAMS.getErrorCode(), EzetapErrors.ERROR_MISSING_MANDATORYPARAMS.getErrorMessage());
                                return;
                            }
            ...
            if (jsonConfigData.has(EzeAPIConstants.KEY_CAPTURE_SIGNATURE)) {
                try {
                    captureSignature = jsonConfigData.getString(EzeAPIConstants.KEY_CAPTURE_SIGNATURE);
            ...
            if (jsonConfigData.has(EzeAPIConstants.KEY_COMMUNICATION_CHANNEL)) {
                try {
                    communicationChannel = getCommunicationChannel(jsonConfigData.getString(EzeAPIConstants.KEY_COMMUNICATION_CHANNEL));
            ...
            if (jsonConfigData.has(EzeAPIConstants.PREPARE_DEVICE)) {
                try {
                    prepareDevice = jsonConfigData.getString(EzeAPIConstants.PREPARE_DEVICE);
                    if ("".equalsIgnoreCase(prepareDevice) || prepareDevice == null) {
                        ezetapErrorCallBack(EzetapErrors.ERROR_MISSING_MANDATORYPARAMS.getErrorCode(), EzetapErrors.ERROR_MISSING_MANDATORYPARAMS.getErrorMessage());
                        return;
            ...
            if (EzetapUserConfig.getPrepareDevice()) {
                ...
                try {
                    EzetapUserConfig.setUserName(userName);
                    result = new JSONObject("{\"message\":\"initialize device successful.\"}");
    ...
    private void initiateWalletPayment(JSONArray pluginArgs) {
        ...
        try {
            ...
            if (amount == null) {
                ezetapErrorCallBack(EzetapErrors.ERROR_MISSING_AMOUNT.getErrorCode(), EzetapErrors.ERROR_MISSING_AMOUNT.getErrorMessage());
                return;
            }
        } catch (Exception e) {
            ezetapErrorCallBack(EzetapErrors.ERROR_MISSING_MANDATORYPARAMS.getErrorCode(), EzetapErrors.ERROR_MISSING_MANDATORYPARAMS.getErrorMessage());
        ...
        try {
            EzetapPayApis.create(EzetapUserConfig.getEzeUserConfig()).startWalletPayment(
                    this,
                    AppConstants.REQ_CODE_PAY_WALLET,
                    EzetapUserConfig.getUserName(),
                    amount,
    ...
    /**
     * Method to initiate Card not present payment
     *
     * @param JSONArray pluginArgs
     *                  - Arguments to the Ezetap API, as passed from JS
     */
    private void initiateCNPPayment(JSONArray pluginArgs) {
        JSONObject jsonObject = null;
        Double amount = null;
        try {
            jsonObject = new JSONObject(pluginArgs.get(0).toString());
        ...
        try {
            EzetapPayApis.create(EzetapUserConfig.getEzeUserConfig()).startCNPPayment(
                    this,
                    AppConstants.REQ_CODE_PAY_WALLET,
                    "http://d.eze.cc/mposui/ezeclick.html",
                    EzetapUserConfig.getUserName(),
                    amount,
                    getOrderNumber(jsonObject),
                    getMobileNumber(jsonObject),
            ...
            e.printStackTrace();
            ezetapErrorCallBack(EzetapErrors.ERROR_API_CNPPAYMENT.getErrorCode(), EzetapErrors.ERROR_API_CNPPAYMENT.getErrorMessage());
        }
    }
    ...
    private void initiateCardPayment(JSONArray pluginArgs) {
        ...
        if (isAmountInvalid(amount, payBackAmount, paymentMode) || paymentMode == null) {
            ...
            try {
                if (EzetapUserConfig.getPrepareDevice() == false) {
    ...
    private void initiateLogin(JSONArray pluginArgs) {
        ...
        try {
            strUserName = new JSONObject(pluginArgs.get(0).toString()).getString(EzeAPIConstants.KEY_USER_NAME);
        } catch (Exception e1) {
        ...
        try {
            strPwd = new JSONObject(pluginArgs.get(0).toString()).getString(EzeAPIConstants.KEY_PASSWORD);
        } catch (Exception e) {
            ezetapErrorCallBack(EzetapErrors.ERROR_DEFAULT_INIT.getErrorCode(), EzetapErrors.ERROR_DEFAULT_INIT.getErrorMessage());
        }
        if (strPwd == null)
    ...
    private Double getAmountCashBack(JSONObject jsonParams) {
        try {
            if (jsonParams.has(EzeAPIConstants.KEY_OPTIONS)) {
                JSONObject jsonOptions = new JSONObject(jsonParams.get(EzeAPIConstants.KEY_OPTIONS).toString());
        ...
        try {
            if (jsonParams.has(EzeAPIConstants.KEY_OPTIONS)) {
                ...
                if (jsonOptions.has(EzeAPIConstants.KEY_REFERENCES)) {
                    JSONObject jsonReferences = new JSONObject(jsonOptions.get(EzeAPIConstants.KEY_REFERENCES).toString());
                    if (jsonReferences.has(EzeAPIConstants.KEY_REFERENCES3)) {
                        String orderNum = jsonReferences.get(EzeAPIConstants.KEY_REFERENCES3).toString();
            ...
            if (jsonParams.has(EzeAPIConstants.KEY_OPTIONS)) {
                ...
                if (jsonOptions.has(EzeAPIConstants.KEY_REFERENCES)) {
                    ...
                    if (jsonReferences.has(EzeAPIConstants.KEY_REFERENCES2)) {
                        String orderNum = jsonReferences.get(EzeAPIConstants.KEY_REFERENCES2).toString();
                        if ("".equalsIgnoreCase(orderNum))
                            return null;
                        else
                            return orderNum;
    ...
    private Double getAmount(JSONObject jsonParams) throws Exception {
        if (jsonParams.has(EzeAPIConstants.KEY_TOTAL_AMOUNT)) {
            Double amt = Double.parseDouble(jsonParams.get(EzeAPIConstants.KEY_TOTAL_AMOUNT).toString());
            if (amt <= 0)
                return null;
            else
                return amt;
            ...
            if (jsonParams.has(EzeAPIConstants.KEY_OPTIONS)) {
                ...
                if (jsonOptions.has(EzeAPIConstants.KEY_REFERENCES)) {
                    ...
                    if (jsonReferences.has(EzeAPIConstants.KEY_REFERENCES1)) {
                        String orderNum = jsonReferences.get(EzeAPIConstants.KEY_REFERENCES1).toString();
                        if ("".equalsIgnoreCase(orderNum))
                            return null;
                        else
                            return orderNum;
        ...
        try {
            if (jsonParams.has(EzeAPIConstants.KEY_OPTIONS)) {
                ...
                if (jsonOptions.has(EzeAPIConstants.KEY_CUSTOMER)) {
                    JSONObject jsonCustomer = new JSONObject(jsonOptions.get(EzeAPIConstants.KEY_CUSTOMER).toString());
                    if (jsonCustomer.has(EzeAPIConstants.KEY_NAME)) {
                        String name = jsonCustomer.get(EzeAPIConstants.KEY_NAME).toString();
    ...
    /**
     * Method to return the labels as a String array for wallet payment from the User Config
     */
    private String[] getLabels() {
        String[] labels = new String[]{"WALLET_USER_" + EzetapUserConfig.getUserName()};
        return labels;
    }
            ...
            if (jsonParams.has(EzeAPIConstants.KEY_OPTIONS)) {
                ...
                if (jsonOptions.has(EzeAPIConstants.KEY_CUSTOMER)) {
                    ...
                    if (jsonCustomer.has(EzeAPIConstants.KEY_MOBILENO)) {
                        String mobileNum = jsonCustomer.get(EzeAPIConstants.KEY_MOBILENO).toString();
            ...
            if (jsonParams.has(EzeAPIConstants.KEY_OPTIONS)) {
                ...
                if (jsonOptions.has(EzeAPIConstants.KEY_CUSTOMER)) {
                    ...
                    if (jsonCustomer.has(EzeAPIConstants.KEY_EMAIL)) {
                        String email = jsonCustomer.get(EzeAPIConstants.KEY_EMAIL).toString();
        ...
        try {
            if (jsonParams.has(EzeAPIConstants.KEY_OPTIONS)) {
                ...
                if (jsonOptions.has(EzeAPIConstants.KEY_AMOUNT_TIP)) {
                    Double tip = Double.parseDouble(jsonOptions.get(EzeAPIConstants.KEY_AMOUNT_TIP).toString());
                    if (tip <= 0)
                        return 0;
                    else
                        return tip;
    ...
    private String getTransactionID(JSONObject jsonParams) throws Exception {
        if (jsonParams.has(EzeAPIConstants.KEY_TRANSACTION_ID)) {
            String txnId = jsonParams.get(EzeAPIConstants.KEY_TRANSACTION_ID).toString();
    ...
    private int getAttachSignFlow(JSONObject jsonParams) {
        if (jsonParams.has(EzeAPIConstants.KEY_IMAGE)) {
            JSONObject imageObj;
            try {
                imageObj = new JSONObject(jsonParams.get(EzeAPIConstants.KEY_IMAGE).toString());
    ...
    private String getEmiID(JSONObject jsonParams) throws Exception {
        if (jsonParams.has(EzeAPIConstants.KEY_EMI_ID)) {
            if (jsonParams.get(EzeAPIConstants.KEY_EMI_ID).toString().equalsIgnoreCase(""))
                return null;
            else
    ...
    private CompressFormat getImageFormat(JSONObject jsonParams) throws Exception {
        ...
        if (imageObj.has(EzeAPIConstants.KEY_IMAGE_TYPE)) {
            if (imageObj.get(EzeAPIConstants.KEY_IMAGE_TYPE).toString().equalsIgnoreCase(""))
                return null;
            else {
                if (imageObj.get(EzeAPIConstants.KEY_IMAGE_TYPE).toString().equalsIgnoreCase("jpeg"))
                    return CompressFormat.JPEG;
                else if (imageObj.get(EzeAPIConstants.KEY_IMAGE_TYPE).toString().equalsIgnoreCase("png"))
                    return CompressFormat.PNG;
                else if (imageObj.get(EzeAPIConstants.KEY_IMAGE_TYPE).toString().equalsIgnoreCase("webp"))
    ...
    private Bitmap getImageBitmap(JSONObject jsonParams) throws Exception {
        ...
        imageObj = new JSONObject(jsonParams.get(EzeAPIConstants.KEY_IMAGE).toString());
        if (imageObj.has(EzeAPIConstants.KEY_IMAGE_DATA)) {
            byte[] decodedString = Base64.decode(imageObj.get(EzeAPIConstants.KEY_IMAGE_DATA).toString(), Base64.DEFAULT);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent intent) {
        switch (requestCode) {
            ...
            case AppConstants.REQ_CODE_PAY_WALLET:
                ...
                break;
            case AppConstants.REQ_CODE_PAY_CNP:
                takeCNPPaymentCallBack(intent, resultCode);
                break;
            ...
            switch (resultCode) {
                case EzeConstants.RESULT_SUCCESS:
                    ...
                    if (WORKFLOW_SELECTED == EzeAPIConstants.WORKFLOW_PREPARE_DEVICE) {
                        ...
                        JSONObject result = new JSONObject("{\"message\":\"Initialize and Prepare device successful.\"}");
                        ezetapSuccessCallBack(result.toString());
                    }
                    break;
    ...
    /**
     * Method to return the response back from the service app for takeWalletPayment API
     *
     * @param int    resultCode
     *               - The result code returned by the service app.
     * @param Intent intent
        ...
        switch (resultCode) {
            case EzeConstants.RESULT_SUCCESS:
                try {
                    ...
                    jsonResult.put(EzeAPIConstants.KEY_TRANSACTION, returnTransactionObject(data.getStringExtra(EzeConstants.KEY_RESPONSE_DATA)));
                    jsonResult.put(EzeAPIConstants.KEY_MERCHANT, returnMerchantObject(data.getStringExtra(EzeConstants.KEY_RESPONSE_DATA)));
                    jsonResult.put(EzeAPIConstants.KEY_CUSTOMER, returnCustomerObject(data.getStringExtra(EzeConstants.KEY_RESPONSE_DATA)));
                    jsonResult.put(EzeAPIConstants.KEY_TRANSACTION_RECEIPT, returnReceiptObject(data.getStringExtra(EzeConstants.KEY_RESPONSE_DATA)));
            ...
            default:
                try {
                    if (data.getStringExtra(EzeConstants.KEY_RESPONSE_DATA) != null) {
                        JSONObject errJson = new JSONObject(data.getStringExtra(EzeConstants.KEY_RESPONSE_DATA));
                        ezetapErrorCallBack(errJson.getString(EzeConstants.KEY_ERROR_CODE), errJson.getString(EzeConstants.KEY_ERROR_MESSAGE));
                    } else {
    ...
    /**
     * Method to return the response back from the service app for takeCNPPayment API
     *
     * @param int    resultCode
     *               - The result code returned by the service app.
     * @param Intent intent
     *               - The result data returned by the service app
     */
    private void takeCNPPaymentCallBack(Intent data, int resultCode) {
        switch (resultCode) {
            case EzeConstants.RESULT_SUCCESS:
                try {
                    JSONObject jsonResult = new JSONObject();
                    ...
                    jsonResult.put(EzeAPIConstants.KEY_CUSTOMER, returnCustomerObject(data.getStringExtra(EzeConstants.KEY_RESPONSE_DATA)));
                    jsonResult.put(EzeAPIConstants.KEY_TRANSACTION_RECEIPT, returnReceiptObject(data.getStringExtra(EzeConstants.KEY_RESPONSE_DATA)));
                    Log.d("success", jsonResult.toString());
                    ezetapSuccessCallBack(jsonResult.toString());
                } catch (Exception e) {
                    ezetapErrorCallBack(EzetapErrors.ERROR_DEFAULT_TAKEPAYMENT.getErrorCode(), EzetapErrors.ERROR_DEFAULT_TAKEPAYMENT.getErrorMessage());
                }
        ...
        switch (resultCode) {
            case EzeConstants.RESULT_SUCCESS:
                try {
                    ...
                    jsonResult.put(EzeAPIConstants.KEY_CUSTOMER, returnCustomerObject(data.getStringExtra(EzeConstants.KEY_RESPONSE_DATA)));
                    jsonResult.put(EzeAPIConstants.KEY_TRANSACTION_RECEIPT, returnReceiptObject(data.getStringExtra(EzeConstants.KEY_RESPONSE_DATA)));
                    jsonResult.put(EzeAPIConstants.KEY_CARD_DETAILS, returnCardObject(data.getStringExtra(EzeConstants.KEY_RESPONSE_DATA)));
                    ezetapSuccessCallBack(jsonResult.toString());
                } catch (Exception e) {
                    ezetapErrorCallBack(EzetapErrors.ERROR_DEFAULT_TAKEPAYMENT.getErrorCode(), EzetapErrors.ERROR_DEFAULT_TAKEPAYMENT.getErrorMessage());
                }
        ...
        try {
            switch (resultCode) {
                case EzeConstants.RESULT_SUCCESS:
                    ...
                    JSONObject jsonResposne = new JSONObject(data.getStringExtra(EzeConstants.KEY_RESPONSE_DATA));
                    if (jsonResposne.has(EzeAPIConstants.KEY_TRANSACTIONS)) {
                        JSONArray temptransactionArray = new JSONArray(jsonResposne.get(EzeAPIConstants.KEY_TRANSACTIONS).toString());
                        for (int i = 0; i < temptransactionArray.length(); i++) {
                            ...
                            tempObj.put(EzeAPIConstants.KEY_CUSTOMER, returnCustomerObject(temptransactionArray.get(i).toString()));
                            tempObj.put(EzeAPIConstants.KEY_TRANSACTION_RECEIPT, returnReceiptObject(temptransactionArray.get(i).toString()));
                            tempObj.put(EzeAPIConstants.KEY_MERCHANT, returnMerchantObject(temptransactionArray.get(i).toString()));
    ...
    private void checkUpdateCallBack(Intent data, int resultCode) {
        try {
            switch (resultCode) {
                case EzeConstants.RESULT_SUCCESS:
                    JSONObject result = new JSONObject("{\"message\":\"Update successful\"}");
                    ezetapSuccessCallBack(result.toString());
                    break;
                case EzeConstants.RESULT_FAILED:
                default:
...
//                EzetapUserConfig.setPrepareDevice(false);
            ...
            default:
                ...
                try {
                    if (data != null) {
                        aJson = new JSONObject(data.getStringExtra(EzeConstants.KEY_RESPONSE_DATA));
                        ezetapErrorCallBack(aJson.get(EzeConstants.KEY_ERROR_CODE).toString(), aJson.get(EzeConstants.KEY_ERROR_MESSAGE).toString());
                    } else {
                        ezetapErrorCallBack(EzetapErrors.ERROR_DEFAULT_LOGOUT.getErrorCode(), EzetapErrors.ERROR_DEFAULT_LOGOUT.getErrorMessage());
                    }
